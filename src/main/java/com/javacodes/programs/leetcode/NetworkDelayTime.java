...
class NetworkDelayTime {
    public static int networkDelayTime(int[][] times, int n, int k) {
        ...
        Queue<Entry> distPq = new PriorityQueue<>(Comparator.comparingInt(Entry::getDist));
//        Set<Integer> notVisited = (Set<Integer>) IntStream.range(1, 10).collect(Collectors.toSet());
        Set<Integer> visited = new HashSet<>();
        int[] distArr  = new int[n];
        Arrays.fill(distArr, Integer.MAX_VALUE);
        distArr[k-1] = 0; // source

//        distPq.add(new Entry(k, 0));
        ...
        adjMap.forEach((a,b)->System.out.println(a+" "+b));

        System.out.println(Arrays.toString(distArr));
        for(int c = 0; c < n; c++){
            Entry source = distPq.poll();

//            distPq.stream().forEach(x -> System.out.println(x.getVertex()+" "+x.getDist()));
            System.out.println(visited);
            System.out.println(distPq.size());
            int currentDist = source.getDist();
            int currentVertex = source.getVertex();
            visited.add(currentVertex);
            System.out.println("\nvertex : " + currentVertex);
            if (adjMap.containsKey(currentVertex)){
                for(Map.Entry<Integer, Integer> adjVertex : adjMap.get(currentVertex).entrySet()){
                    if(!visited.contains(adjVertex.getKey()) && currentDist + adjVertex.getValue() < distArr[adjVertex.getKey()-1]){
                        distArr[adjVertex.getKey()-1] = currentDist + adjVertex.getValue();
                        distPq.offer(new Entry(adjVertex.getKey(), currentDist + adjVertex.getValue()));
                    }
                }
            }
            System.out.println(Arrays.toString(distArr));
        ...
        System.out.println(Arrays.toString(distArr));
        int signal = Arrays.stream(distArr).max().getAsInt();
        if (signal == Integer.MAX_VALUE)
            return -1;
        else
            return signal;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(NetworkDelayTime.networkDelayTime(ip,15, 2));
    }
